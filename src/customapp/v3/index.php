...
if (isset($_SESSION["access_token"]) && isset($_SESSION["refresh_token"])) { // we already have a session.
    ...
    if ($result = $rh->getProfileDataFromMyOAuthProvider($access_token)) {
        ...
        error_log("Unable to get profile data with access token $access_token. Trying to refresh token...");
        if ($newTokenData = $rh->refreshAccessToken($refresh_token)) {
            ...
            error_log("Trying to get profile info again with refreshed token: $access_token");
            if ($result2 = $rh->getProfileDataFromMyOAuthProvider($access_token)) {
                $name = $result2["name"];
                $email = $result2["email"];
                error_log("Success! name: $name, email: $email");
            } else { error_log("I don't seem to be able to get the profile information with the new token either."); }
        } else { error_log("Error refreshing access token with refresh token $refresh_token."); }
    }
}
?>
<!DOCTYPE html>
